...
// Use API composable
const { getRandomWords, submitLearningRecord, loading, error } = useAPI()
...
onMounted(async () => {
  const fetchedWords = await getRandomWords(undefined, undefined, 10)
  if (fetchedWords.length > 0) {
    allWords.value = fetchedWords
    words.value = [...fetchedWords]
  } else {
    // Fallback to sample data if API fails
    const sampleWords = [
      { id: 1, chinese: '蘋果', english: 'apple', pinyin: 'píng guǒ' },
      { id: 2, chinese: '香蕉', english: 'banana', pinyin: 'xiāng jiāo' },
      { id: 3, chinese: '橙', english: 'orange', pinyin: 'chéng' },
      { id: 4, chinese: '書包', english: 'school bag', pinyin: 'shū bāo' },
      { id: 5, chinese: '鉛筆', english: 'pencil', pinyin: 'qiān bǐ' },
      { id: 6, chinese: '老師', english: 'teacher', pinyin: 'lǎo shī' },
      { id: 7, chinese: '學生', english: 'student', pinyin: 'xué shēng' },
      { id: 8, chinese: '爸爸', english: 'father', pinyin: 'bà ba' },
      { id: 9, chinese: '媽媽', english: 'mother', pinyin: 'mā ma' },
      { id: 10, chinese: '太陽', english: 'sun', pinyin: 'tài yáng' }
    ]
    allWords.value = sampleWords
...
const nextWord = () => {
  ...
  startTime.value = Date.now() // Reset timer for next word
}
